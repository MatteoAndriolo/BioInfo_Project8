...
if __name__ == "__main__":
    ...
    for f_path in files_path:
        ...
        p_cut = "S0R\d*\t(.*)"
        taxid_kraken = re.findall(
            p_cut, open(f_path.replace(".fna", ".cut").replace("_Ref", ""), "r").read()
        )
        ...
        jsonData[name] = {
            ...
            "taxid": sorted(
                [
                    [t, taxid_kraken.count(t) / len(taxid_kraken)]
                    for t in set(taxid_kraken)
                ],
                key=lambda x: x[1],
                reverse=True,
            ),
            "id": [[a, c] for a, c in zip(taxid_kraken, description_lines)]
            if len(set(taxid_kraken)) != 1
            else taxid_kraken[0]
